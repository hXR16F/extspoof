...
copy "%q1%" "backup_%q1%"
if exist "spoof.exe" (spoof.exe "%filename%" "%extension%" "%spoofed_filename%" "%spoofed_extension%" "%dot%") else (python spoof.py "%filename%" "%extension%" "%spoofed_filename%" "%spoofed_extension%" "%dot%")
